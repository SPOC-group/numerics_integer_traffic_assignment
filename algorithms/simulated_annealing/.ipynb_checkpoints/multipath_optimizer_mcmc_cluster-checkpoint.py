...
def Plot_network(G_traffic,pos="circular"): 
    to_plot_graph = nx.Graph()
    for i in range(N):
        to_plot_graph.add_node(i)
    for u,v in nx_G.edges():
        to_plot_graph.add_edge(u,v,weight=(G_traffic.weight(u,v)))

    weights = [(G_traffic.weight(u,v)) for u,v in G_traffic.iterEdges()]
    if(pos=="spring"):
        pos= nx.spring_layout(to_plot_graph)
    elif(pos=="spectral"):
        pos= nx.spectral_layout(to_plot_graph)
    elif(pos=="random"):
        pos= nx.random_layout(to_plot_graph)
    if(pos=="planar"):
        pos= nx.planar_layout(to_plot_graph)
    else:
...
def annealed_optimizer(G,start_nodes,end_nodes,beta_schedule,phi,mcmc_steps=10,mcmc_seed=None,quiet=True, conv_iters=3):
    ...
    for u,v in Gd.iterEdges(): #initializing cost on all the graph: this way one will only have to modify the cost on the edges traversed by the considered path
        Gd.setWeight( u,v,  M * phi((G_traf.weight(u,v)+1)/M)-M * phi(G_traf.weight(u,v)/M))
    total_energy=M*sum([phi(w/M) for _,_,w in G_traf.iterEdgesWeights()]) #this is the objective functino to minimize. When it becomes stationary, the algorithm stops. 
    count_conv=0
    ...
    for t,beta in enumerate(beta_schedule):
        ...
        for nu in range(M):
            ...
            for i in range(len(path)-1): 
                ...
                Gd.setWeight(path[i],path[i+1],M*phi((G_traf.weight(path[i],path[i+1])+1)/M)- M*phi((G_traf.weight(path[i],path[i+1]))/M))
                Gd.setWeight(path[i+1],path[i],M*phi((G_traf.weight(path[i],path[i+1])+1)/M)- M*phi((G_traf.weight(path[i],path[i+1]))/M))
            ...
            for i in range(len(new_path)-1): #add the contribution of the new path nu
                ...
                Gd.setWeight(new_path[i],new_path[i+1], M* phi((G_traf.weight(new_path[i],new_path[i+1])+1)/M)- M*phi((G_traf.weight(new_path[i],new_path[i+1]))/M)) 
                Gd.setWeight(new_path[i+1],new_path[i], M* phi((G_traf.weight(new_path[i],new_path[i+1])+1)/M)- M*phi((G_traf.weight(new_path[i],new_path[i+1]))/M)) 
                
        #check for convergence in energy
        new_total_energy=M*sum([phi(w/M) for _,_,w in G_traf.iterEdgesWeights()])
