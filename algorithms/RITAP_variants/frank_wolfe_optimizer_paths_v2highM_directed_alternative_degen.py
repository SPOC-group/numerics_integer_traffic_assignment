...
# this program inplements the Frank-Wolfe method to solve the traffic assignment problem. It also implements the RITAP algorithm
#compared to other methods, it uses a different approach to compute support paths. In short instead of looking at shortest paths in the graph weighted with phi'(I_e) it simply takes the paths on which flow is allocated.
...
#def phi(x, pow, b, cap, t0):
#    return t0*x*(1+(b/(pow+1))*(x/cap)**pow)

def phi(x,gamma):
    return x**gamma


#def deriv_phi(x,pow, b, cap , t0):
#    return t0*(1+b*(x/cap)**pow)
...
def num_paths_tap(paths_dict, flow_eps=0):
    """
    Computes the number paths on which positive traffic is allocated.
    Compare this with the function 'support_paths' whose first line says 'Computes the paths on which positive traffic is allocated'
    ...
    flow_eps: if a path carries less than flow_eps units of flow, it is not counted in the paths
    Returns:
    the total number of paths over which flow is routed

    """
    tot_degen=0
    for start,end in paths_dict:
        for flow in paths_dict[start,end][1]:
            if flow>flow_eps:
                tot_degen+=1
...
parser.add_argument(
    "--no_output_flow_paths",
     action='store_true',
    help="If this flag is give, the algorithm will not output the final edge flows and the final path flows",
)
...
if not quiet:
    print(f"computing relevant statistics... elapsed time {time.time()-start_time}")


rgap=RGAP(G,G_traf,bushes)
result = re.search(r'desPairs_(\d+)_M', sources_destinations_filename)
seed = int(result.group(1)) #this is the seed used to generate the graph
...
if(not quiet):
    print(f"computing degeneracy... elapsed time {time.time()-start_time}")
for rtol in [10**(-x) for x in range(0)]: #does not execute
...
log_flow_thresholds= [2,3,4,5,6,9]
tot_num_tap_paths=[]
for log_flow_thr in log_flow_thresholds:
    tot_num_tap_paths.append(num_paths_tap(paths_dict, flow_eps=10**(-log_flow_thr)))


header="N M d gamma tmax seed FW_fin_energy FW_fin_opt_gap int_FW_path_energy shortest_path_energy frac_shortest_paths avg_traf_int_paths avg_len_int_paths time avg_len_shortest_path avg_traf_shortest_paths avg_traf_FW avg_traf_int_FW RGAP avg_degen_od_FW identifier num_start_end_pairs "+" ".join([f"misall_traffic_1e-{x}" for x in range(1,6)])+" "+" ".join([f"sup_paths_od_degen_rtol_1e-{x}" for x in range(0)])+" "+" ".join([f"frac_integer_paths_tol_{x:1.0e}" for x in tol_integer_paths])+" frac_FW_flow_SP "+" ".join([f"frac_sup_shortest_paths_tol_1e-{x}" for x in range(0)])+" final_t init_energy "+" ".join([f"tot_num_tap_paths_tol_1e-{x}" for x in log_flow_thresholds])+"\n"
if not quiet:
    print(f"writing final results on file...elapsed time {time.time()-start_time}")
