...
# this program inplements the Frank-Wolfe method to solve the traffic assignment andthe integer traffic assignment problems.
...
#BPR function
def phi(x,args):
 # args is [cap,t0,b,power]
    return args[1]*x*(1+(args[2]/(args[3]+1))*(x/args[0])**args[3])

#def phi(x,gamma):
#    return x**gamma


def deriv_phi(x,args):
    # args is [cap,t0,b,power]
    return args[1]*(1+args[2]*(x/args[0])**args[3])
...
def FW_traffic_assignment_single_commodity(bushes, G, lr, tmax, phi, deriv_phi, param_BPR_dict, rtol=1e-7, rel_opt_gap_thr=0,first_thru_node=0):
    ...
    WARNING, NOT IMPLEMENTED: first_thru_node (int): index of the first node in the graph through which flow is allowed to pass. Usually the first nodes correspond to zones, these are terminal points for the flow, but they should not be used as intermediate routing nodes 
    #this function lacks supprot for first thru node
    ...
    for u, v in G.iterEdges():
        G.setWeight(u, v,  param_BPR_dict[u,v][1])
    ...
    for t in range(tmax):
        ...
        total_energy = sum([phi(w,param_BPR_dict[e1,e2]) for e1, e2, w in G_traf.iterEdgesWeights()])
        ...
        for u, v in G.iterEdges():
            G.setWeight(u, v, deriv_phi(G_traf.weight(u, v),param_BPR_dict[u,v]))
...
def continuous_to_max_traf_integer_paths(paths_dict, bushes):
    ...
    WARNING:this functino will give nonsense results if the demand matrix is not integer
    ...
    for start,end in paths_dict:
        ...
        if(allocated_flow<bushes[start][end]): #now adding the fractional parts if necessary
            ...
            for k in range(int(bushes[start][end])-allocated_flow):
...
parser.add_argument(
    ...
    "--network_file",
    type=str,
    help="path to the txt input file containing the network structure",
)
parser.add_argument(
    "-pairs",
...
parser.add_argument(
    "--no_integer_paths",
     action='store_true',
    help="If this flag is given, the algorithm will not attempt to project back onto integer paths. This option is normally useful when dealing with noninteger demand matrices, which would otherwise break the code",
)
...
network_filename = args.network_file
...
no_integer_paths=args.no_integer_paths
...
param_BPR_dict={}
first_thru_node=None
N=None #number of nodes
file=open(network_filename, 'r')
net_file_separator='\t'
while(True):
    line=file.readline()
    elements = line.strip().split(net_file_separator)
    elements = [elem.strip() for elem in elements if elem.strip()]
    match = re.search(r'<NUMBER OF NODES>\s+(\d+)', line.strip())
    if match:
        N= int(match.group(1))

    match = re.search(r'<FIRST THRU NODE>\s+(\d+)', line.strip())
    if match:
        first_thru_node= int(match.group(1))-1

    if(len(elements)>0 and elements[0]=='~'): #the tilde is present in the header so everything after that is data to be read
        break
   
if(N is None):
    raise Exception("ERROR: was unable to find number of nodes in graph from metadata. Please in the network_file include a line at the beginning with '<NUMBER OF NODES> N', where N is the number of nodes")

#creating the graph
G = nk.Graph(n=N, weighted=True, directed=True, edgesIndexed=False)

num_edges=0 #number of directed edges
while(True):
    line=file.readline()
    if not line:
    ...
    elements = [elem.strip() for elem in elements if elem.strip()]
    v0=int(elements[0])-1 #indexing starts at zero in my case
    v1=int(elements[1])-1
    param_BPR_dict[(v0,v1)]=[float(elements[2]),float(elements[4]),float(elements[5]),float(elements[6]),float(elements[7])]
    G.addEdge(v0, v1)  # in the input file the nodes start from 1, we want them to start from zero.
    G.setWeight(v0, v1, 1)
    num_edges+=1
...
if(first_thru_node>1):
    print("WARNING: traffic through some zones is forbidden. This may cause buggy behavior. See https://github.com/bstabler/TransportationNetworks/ for meaning of first_thru_node")
...
with open(sources_destinations_filename, "r") as sources_destinations_file:
    bushes=json.load(sources_destinations_file) 

#converting the json keys which are read as strings, into integers.
bushes= {int(key): value for key, value in bushes.items()} #OD pairs are indexed starting from zero already in the json input file
for s in bushes:
...
start_time=time.time()
(G_traf,G,dict_paths,energies,opt_gaps,flag_conv,final_t) = FW_traffic_assignment_single_commodity(bushes,G,lr,tmax,phi,deriv_phi,param_BPR_dict,rtol=rtol,rel_opt_gap_thr=rel_opt_gap_thr)  # compute the flow of each commodity
...
for u,v in G_sp.iterEdges(): #setting weights to free flow travel times
    G_sp.setWeight(u,v, param_BPR_dict[u,v][1])
...
for s in bushes:
    ...
    for e in bushes[s]:
        ...
        idx_integer_shortest_paths=np.where(lengths_integer_paths_se == len(shortest_path_se))[0]
...
shortest_paths_energy=sum([phi(w,param_BPR_dict[u,v]) for u,v,w in G_sp_traf.iterEdgesWeights()])
integer_paths_energy = sum([phi(w, param_BPR_dict[u,v]) for u,v,w in G_traf_int.iterEdgesWeights()])
...
for rtol in [10**(-x) for x in range(3,6)]:
